        ...
        <div class="container">
            <div class="row">
                ...
                @foreach($category_name as $key => $category_name)
                            ...
                            <div class="col-lg-8 col-md-4" style="margin-top:25px">
                                <div class="filter__found">
                                    <form style="display:flex">
                                        ...
                                        <input type="hidden" name="start_price" id="start_price" >
                                        <input type="hidden" name="end_price" id="end_price" >
                                        <input type="submit" name="filter_price" value="Lọc giá" class="btn btn-default btn-sm" style="margin-left:15px">
                                    </form>
                    ...
                    <div class="row">
                        @foreach($category_by_id as $key => $cbi)
                        <div class="col-lg-4 col-md-6 col-sm-6">
                                <a id="wishlist_producturl{{$cbi->product_id}}" href="{{URL::to('/chi-tiet-san-pham/'.$cbi->product_id)}}">
                                <form>
                                    <input type="hidden" name="_token" value="{{csrf_token()}}">
                                    <input type="hidden" class="cart_product_id_{{$cbi->product_id}}" value="{{$cbi->product_id}}">
                                    <input type="hidden" id="wishlist_productname{{$cbi->product_id}}" class="cart_product_name_{{$cbi->product_id}}" value="{{$cbi->product_name}}">
                                    <input type="hidden" class="cart_product_image_{{$cbi->product_id}}" value="{{$cbi->product_image}}">
                                    <input type="hidden" id="wishlist_productprice{{$cbi->product_id}}" class="cart_product_price_{{$cbi->product_id}}" value="{{$cbi->product_price}}">
                                    <input type="hidden" class="cart_product_quantity_{{$cbi->product_id}}" value="{{$cbi->product_quantity}}">
                                    <input type="hidden" class="cart_product_qty_{{$cbi->product_id}}" value="1">
                                </form>
                            <div class="product__item">
                                <div class="product__item__pic set-bg" style ="border:1px solid;border-radius: 3px;">
                                    <img id="wishlist_productimage{{$cbi->product_id}}" src="{{URL::to('uploads/product/'.$cbi->product_image)}}" alt="" />
                                ...
                                <div class="product__item__text">
                                    <h6 ><a href="#">{{$cbi->product_name}}</a></h6>
                                    <h5 style="color:red">{{number_format($cbi->product_price).' '.'VNĐ'}}</h5>
                                </div>
                                    <style>
                                    ...
                                    <button type="button" class="btn add-to-cart" name="add-to-cart" data-id_product="{{$cbi->product_id}}"
                                    ...
                                    <button class="btn btn-default button_wishlist" id="{{$cbi->product_id}}"
                                    ...
                                    class="btn btn-default xemnhanh" data-id_product="{{$cbi->product_id}}" 
                                    data-id_product="{{$cbi->product_id}}" name="" >Xem nhanh</button>
                            </div>
                            <a>
                        </div>
                        @endforeach
