...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            $fetch = Admin::where('admin_id', $request['admin_id'])->first();
        }
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch,
            'user_avatar' => asset('/storage/' . config('chatify.user_avatar.folder') . '/' .'avatar.png'),
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('tbl_admin',  function ($join) {
            $join->on('messages.from_id', '=', 'tbl_admin.admin_id')
                ->orOn('messages.to_id', '=', 'tbl_admin.admin_id');
        })
            ->where('messages.from_id', Auth::user()->admin_id)
            ->orWhere('messages.to_id', Auth::user()->admin_id)
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $user) {
                if ($user->id != Auth::user()->admin_id) {
                    // Get user data
                    $userCollection = Admin::where('admin_id', $user->admin_id)->first();
    ...
    {
        // Get user data
        $userCollection = Admin::where('admin_id', $request['user_id'])->first();
    ...
    {
        $favoritesList = null;
        $favorites = Favorite::where('user_id', Auth::user()->admin_id);
        foreach ($favorites->get() as $favorite) {
            // get user data
            $user = Admin::where('admin_id', $favorite->favorite_id)->first();
    ...
    public function search(Request $request)
    {
        ...
        $records = Admin::where('admin_name', 'LIKE', "%{$input}%");
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? Admin::where('admin_id', Auth::user()->admin_id)->update(['dark_mode' => 1])  // Make Dark
                : Admin::where('admin_id', Auth::user()->admin_id)->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request['messengerColor']) {
            ...
            Admin::where('admin_id', Auth::user()->admin_id)
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (Auth::user()->admin_avatar != config('chatify.user_avatar.default')) {
                    ...
                    $update = Admin::where('admin_id', Auth::user()->admin_id)->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        $update = $request['status'] > 0
            ? Admin::where('admin_id', $request['user_id'])->update(['active_status' => 1])
            : Admin::where('admin_id', $request['user_id'])->update(['active_status' => 0]);
        // send the response
        return Response::json([
            'status' => $update,
        ], 200);
